...
# GATES the publish. Shared with the multi-arch leg builder via smoke.sh so the
# two publish paths cannot drift apart again (the noarch path had no check at all,
# which is how scanpy #63 and humann 3.9 shipped broken).
echo "[build] smoke test (arm64) ..."
if ! "$HERE/smoke.sh" "$TMP_IMAGE" "$PKG" linux/arm64; then
  echo "[build] not publishing ${PKG}=${VER}" >&2
